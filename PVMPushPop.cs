...
*/
					// cases we were given missing from print out
                    case PVM.inpc:
                        var address_for_char = Pop();
                        var char_in = data.ReadChar(); 
                        //Push(data.ReadChar());
                        mem[address_for_char] = (int)char_in;
                    ...
                    case PVM.low: // toLowerCase
                        int c_low = Pop();
                        c_low = (int)char.ToLower((char)c_low);
                        Push((int)c_low);
                        break;
                    case PVM.inc:           // ++
                        int inc_a = Pop() + 1;
                    ...
                    if (InBounds(adr)) Push(adr);
                        break;
                    case PVM.ldl:
                    var constantUser=Pop();           // push local value               
                     results.WriteLine("\nLDL N = " + constantUser);
                    adr = cpu.fp -1-constantUser;
                    if (InBounds(adr))Push(mem[adr]);
                        break;
                    case PVM.ldl_0:         // push value of local variable 0
                    adr = cpu.fp-1;
                    results.WriteLine("address= " + adr.ToString() + "\n value= " + mem[adr]);
                    if (InBounds(adr))Push(mem[adr]);
                        break;
                    ...
                    case PVM.stl:           // store local value
                        tos = Pop(); adr = cpu.fp-Pop();
                        results.WriteLine("Tos= " + tos + "\n adr= " + adr);
                    ...
                    case PVM.stl_2:         // pop to local variable 2
                        tos = Pop(); adr = cpu.fp - 3;
                        results.WriteLine("Tos= " + tos + "\n adr= " + adr);
                        if (InBounds(adr)) mem[adr] = tos;
                        results.WriteLine("\nSaved Value= " + mem[adr]);
                        break;
                    case PVM.stl_3:         // pop to local variable 3
                        tos = Pop(); adr = cpu.fp - 4;
                        results.WriteLine("Tos= " + tos + "\n adr= " + adr);
                        if (InBounds(adr)) mem[adr] = tos;
                        results.WriteLine("\nSaved Value= " + mem[adr]);
                        break;
                    ...
                    case PVM.cap:           // toUpperCase
                        int c_Upper = Pop();
                        char tempc = (char)c_Upper;
                        char up = char.ToUpper(tempc);
                        c_Upper = (int)up;
                        Push(c_Upper);
                        break;
                    //     case PVM.low:           // toLowerCase
                                    ...
                                    bool traceStack, bool traceHeap)
        {
            ...
            do
            {
                ...
                {         // execute
                    ...
                    case PVM.sto:           // store
                        tos = Pop(); adr = Pop();
                        results.WriteLine("Address read in: " + adr.ToString() + "\n Value readIn: " + tos.ToString());
                        if (InBounds(adr)) mem[adr] = tos;
                        break;
                    ...
                    case PVM.prnb:          // boolean output
                        if (tracing) results.Write(padding);
                        var a = Pop();
                        if (a != 0) results.Write(" true  "); else results.Write(" false ");
                        if (tracing) results.WriteLine();
                        break;
                    case PVM.inpc:
                        var address_for_char = Pop();
                    ...
                    case PVM.mul:           // integer multiplication
                        ...
                        {
                            PostMortem(results, mulOverFlow);
                            break;
                        }
                        Push(temp * tos);
                        break;
                    case PVM.div:           // integer division (quotient)
                        ...
                        if (tos == 0)
                        {
                            PostMortem(results, PVM.divZero);
                            break;
                        }
                        Push(Pop() / tos);
                        break;
                    ...
                    case PVM.islet:
                        int val = Pop();
                        if (Char.IsLetter((char)val)) Push(1); else Push(0);
                        break;
                    case PVM.low: // toLowerCase
                        int c_low = Pop();
                        c_low = (int)char.ToLower((char)c_low);
